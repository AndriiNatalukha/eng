...
import styles from './TestWordsReverse.module.scss';
...
const TestWordsReverse: React.FC<Props> = ({ packetWords, auth }) => {
    debugger
    ...
    const handleClick = (id: number) => {
        if (curWord.id === id) {
            ...
            toast.success('Вірно');
            setCounts({ ...counts, correct: counts.correct + 1 });
        } else {
            ...
            setCounts({ ...counts, wrong: counts.wrong + 1 });
        }
        ...
        <div className={styles.container}>
            ...
            <div className={`${styles.modal}`}>
                {packetWords.length > 2 && (
                    <div>
                        <h1 className={`${styles.h1} text-2xl font-bold mb-4`}>{curWord.ukrainian_word}</h1>
...
export default TestWordsReverse;
